...
go tool oapi-codegen --config "$CONFIG" -generate models -o internal/api/types.gen.go -package api "$SPEC"

echo "[2/5] generating internal/api/spec.gen.go (gin-server + strict-server)"
go tool oapi-codegen --config "$CONFIG" -generate 'gin-server,strict-server' -o internal/api/spec.gen.go -package api "$SPEC"

echo "[3/5] generating pkg/api/types.gen.go (client-side models)"
go tool oapi-codegen --config "$CONFIG" -generate models -o pkg/api/types.gen.go -package api "$SPEC"

echo "[4/5] generating pkg/api/client.gen.go (client)"
go tool oapi-codegen --config "$CONFIG" -generate client -o pkg/api/client.gen.go -package api "$SPEC"

echo "[5/5] generating pkg/api/spec.gen.go (embedded OAS document)"
go tool oapi-codegen --config "$CONFIG" -generate spec -o pkg/api/spec.gen.go -package api "$SPEC"
