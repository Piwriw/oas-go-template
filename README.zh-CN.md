...
HTTP 服务默认使用 5 秒读请求头超时、15 秒读超时、30 秒写超时和 60 秒空闲超时，限制请求头为 1 MiB、请求体为 1 MiB。收到关闭信号后，服务会关闭监听器，并通过 `http.Server.Shutdown` 给在途请求最多 10 秒完成。可在 `config.yaml` 的 `server` 下调整 HTTP 超时；流式响应可将 `write_timeout: 0`，应用层请求体限制可将 `max_body_mb: 0` 关闭。
...
服务会校验下线日期晚于弃用日期。接口应持续可用到下线日期；提前删除会被视为破坏性变更。
